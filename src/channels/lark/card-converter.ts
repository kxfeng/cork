/**
 * Lark interactive (card) message converter.
 *
 * The CardConverter class is a near-verbatim port of openclaw-lark's
 * src/messaging/converters/interactive/card-converter.ts. Its dependency
 * modules (types, card-utils, utils, legacy) and the top-level convertCard /
 * extractCardImageKeys entry points are inlined so cork carries one
 * self-contained file.
 *
 * Original work:
...
function safeParse(s: unknown): unknown {
  ...
  // The raw_card_content envelope delivers json_card as a string but
  // json_attachment as an already-parsed object — accept both.
...
function formatMillisecondsToISO8601(milliseconds: string): string {
  const ms = parseInt(milliseconds, 10);
  if (isNaN(ms)) return '';
  return new Date(ms).toISOString();
}

function normalizeTimeFormat(input: string): string {
  if (!input) return '';
...
export class CardConverter {
  ...
  convertImage(prop: Obj, _id: string): string {
    ...
    // Emit a [image: <key>] token so the caller can swap in the downloaded
    // local path. The key matches what extractCardImageKeys() returns.
    const imageKey = this.resolveImageKey(prop);
    if (imageKey) result += ` [image: ${imageKey}]`;

    return result;
  }

  /**
   * Resolve a card image element to its real downloadable key. Card `img`
   * elements reference the key indirectly via json_attachment.images.
   */
  private resolveImageKey(prop: Obj): string {
    const direct = prop.img_key as string | undefined;
    if (direct) return direct;
    const imageID = prop.imageID as string | undefined;
    if (!imageID) return '';
    if (this.attachment) {
      const images = this.attachment.images as Obj | undefined;
      const info = images?.[imageID] as Obj | undefined;
      if (info && typeof info === 'object') {
        const key = (info.origin_key as string) || (info.token as string);
        if (key) return key;
      }
    }
    // imageID may itself already be a real image key.
    return imageID.startsWith('img_') || imageID.startsWith('img-')
      ? imageID
      : '';
  }
...
// ---------------------------------------------------------------------------
// Legacy fallback + top-level entry points
// ---------------------------------------------------------------------------

/**
 * Extract text from a card that is NOT in raw_card_content envelope form —
 * e.g. a template card, or the degraded placeholder the WebSocket event
 * delivers. Best-effort: pulls markdown / plain_text / div text and recurses.
 */
function convertLegacyCard(parsed: Obj): string {
  const texts: string[] = [];
  ...
  extractLegacyTexts(elements, texts);

  return texts.join('\n');
}

function extractLegacyTexts(elements: unknown[], out: string[]): void {
    ...
    if (elem.tag === 'column_set') {
      ...
      if (columns) {
        for (const col of columns) {
          ...
          if (colObj.elements) extractLegacyTexts(colObj.elements as unknown[], out);
        }
      }
    }
    if (elem.elements) extractLegacyTexts(elem.elements as unknown[], out);
  }
}

/**
 * Convert a Lark interactive (card) message into plain text for the LLM.
 *
 * `rawContent` is the message body content string. When fetched with
 * `card_msg_content_type=raw_card_content` it is a `{json_card,...}` envelope
 * and gets the full CardConverter treatment; otherwise it falls back to a
 * best-effort legacy extraction.
 */
export function convertCard(rawContent: string): string {
  const parsed = safeParse(rawContent) as Obj | undefined;
  if (!parsed || typeof parsed !== 'object') return '(card message)';

  if (typeof parsed.json_card === 'string') {
    const result = new CardConverter(MODE.Concise).convert(
      parsed as unknown as RawCardContent,
    );
    return result.content || '(card message)';
  }

  const legacy = convertLegacyCard(parsed).trim();
  return legacy || '(card message)';
}

/**
 * Collect every downloadable image key referenced by a card.
 *
 * Card `img` elements carry an `imageID` that is a reference into
 * `json_attachment.images` — the real downloadable key is the entry's
 * `origin_key` (e.g. `img_v3_...`). Numeric refs with no attachment entry
 * are dropped; refs that are already real `img_*` keys are kept as-is.
 */
export function extractCardImageKeys(rawContent: string): string[] {
  const env = safeParse(rawContent) as Obj | undefined;
  if (!env) return [];

  const card = safeParse(env.json_card) as Obj | undefined;
  if (!card || typeof card !== 'object') return [];

  const attachment = safeParse(env.json_attachment) as Obj | undefined;
  const imageMap =
    attachment && typeof attachment.images === 'object'
      ? (attachment.images as Obj)
      : undefined;
  ...
  // Resolve an imageID reference to its real downloadable key.
  const resolve = (ref: unknown): void => {
    if (typeof ref !== 'string' || !ref) return;
    const info = imageMap?.[ref] as Obj | undefined;
    if (info && typeof info === 'object') {
      push((info.origin_key as string) || (info.token as string));
      return;
    }
    // Already a real image key (no attachment indirection).
    if (ref.startsWith('img_') || ref.startsWith('img-')) push(ref);
  };
  ...
  const visit = (node: unknown): void => {
    ...
    if (tag === 'img' || tag === 'image') {
      const p =
        o.property && typeof o.property === 'object' ? (o.property as Obj) : o;
      resolve(p.imageID);
      resolve(p.img_key);
      resolve(o.img_key);
    } else if (tag === 'img_combination') {
      const p =
        o.property && typeof o.property === 'object' ? (o.property as Obj) : o;
      const list = p.imgList;
      if (Array.isArray(list)) {
        for (const img of list) {
          if (img && typeof img === 'object') resolve((img as Obj).imageID);
        }
      }
    }
